...
public class FirstTests extends TestBase {
	...
	@Test
	public void test001() {
		HashMap<String, Object> value = SerenityRest.rest().given().when().get("/organization/getAllOrganizations")
				.then().log().all().statusCode(200).extract().path("findAll{it}.get(0)");
	...
	@Test
	public void test002() {
		SerenityRest.rest().given().when().get("/organization/getOrganization/" + id).then().log().all()
				.statusCode(200);
	}
	...
	public void test003() {
		SerenityRest.rest().given().when()
				.get("/organization/getOrganizationByUserName?userName=admin.test@synectiks.com").then().log().all()
				.statusCode(200);
	}
}
